...
TEST_F(GraspTest, k4) {
    ASSERT_EQ(grasp(k4, 0.1, 10, 0).outgoing, (unsigned int)4);
    ASSERT_EQ(grasp(k4, 0.3, 10, 0).outgoing, (unsigned int)4);
    ASSERT_EQ(grasp(k4, 0.5, 10, 0).outgoing, (unsigned int)4);
    ASSERT_EQ(grasp(k4, 0.7, 10, 0).outgoing, (unsigned int)4);
}

TEST_F(GraspTest, k4PlusTwoEdges) {
    ASSERT_EQ(grasp(k4plus, 0.1, 10, 0).outgoing, (unsigned int)6);
    ASSERT_EQ(grasp(k4plus, 0.3, 10, 0).outgoing, (unsigned int)6);
    ASSERT_EQ(grasp(k4plus, 0.5, 10, 0).outgoing, (unsigned int)6);
    ASSERT_EQ(grasp(k4plus, 0.7, 10, 0).outgoing, (unsigned int)6);
}

TEST_F(GraspTest, allSmall) {
    std::vector<testcase> tests(getTests(true));
    for (size_t i = 0; i < tests.size(); ++i) {
        cliqueInfo result = grasp(tests[i].input, 0.1, 10, 0);
...
TEST_F(GraspTest, allHuge) {
    ...
    for (size_t i = 0; i < tests.size(); ++i) {
        cliqueInfo result = grasp(tests[i].input, 0.3, 10, 0);
...
TEST_F(GraspTest, allHugeTimeLimit) {
    std::vector<testcase> tests(getTests(false));
    for (size_t i = 0; i < tests.size(); ++i) {
        cliqueInfo result = grasp(tests[i].input, 0.3, 0, 0.00001);
        nodeSet resultNodes = result.nodes;
        ASSERT_LE(result.outgoing, tests[i].output.outgoing) << "Caso huge-" << i;
        //ASSERT_GE(result.outgoing, localSearchHeuristic(tests[i].input).outgoing) << "Caso huge-" << i;
        if(result.outgoing == tests[i].output.outgoing) {
